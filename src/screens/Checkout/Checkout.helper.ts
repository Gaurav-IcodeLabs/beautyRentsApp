...
import {INSTANT_BOOKING_TYPE, minutesBetween} from '../../util';
...
const fetchSpeculatedTransactionIfNeeded = (
  ...
  const isDayOrHourBooking = pageData?.orderData?.isDayOrHourBooking;
  orderParams.isDayOrHourBooking = isDayOrHourBooking;
  const processName =
...
export const loadInitialDataForStripePayments = ({
  ...
  // if (pageData?.orderData?.couponCode) {
  //   orderParams.couponCode = pageData.orderData.couponCode;
  // }
...
export const processCheckoutWithPayment = (
  ...
  const {isDayOrHourBooking, couponCode = null} = pageData?.orderData;
  const processAlias =
    pageData?.listing?.attributes?.publicData?.transactionProcessAlias;
  const bookingType = pageData?.listing?.attributes?.publicData?.bookingType;
  const isInstantBooking = bookingType === INSTANT_BOOKING_TYPE;
  let createdPaymentIntent = null as any;
  let stripePaymentIntentId = null as any;
  ////////////////////////////////////////////////
  ...
  const fnRequestPayment = fnParams => {
    ...
    fnParams.isDayOrHourBooking = isDayOrHourBooking;
    fnParams.couponCode = couponCode;
  ...
  const fnConfirmPayment = fnParams => {
    ...
    if (stripePaymentIntentId) {
      params = {
        ...
        params: {
          isInstantBooking,
          protectedData: {
            sharetribePaymentIntent: stripePaymentIntentId,
          },
        },
      };
    }
    ...
    return isTransitionedAlready
      ? Promise.resolve(storedTx)
      : dispatch(confirmPayment(params)).then(res => {
          return res.payload;
        });
  };
