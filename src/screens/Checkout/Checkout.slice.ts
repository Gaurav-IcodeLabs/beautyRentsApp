...
export const confirmPayment = createAsyncThunk(
  ...
  async (orderParams, {dispatch, extra: sdk}) => {
    ...
    return sdk.transactions
      ...
      .then(order => {
        // if instant booking is enabled, transition with instant booking
        if (orderParams?.params?.isInstantBooking) {
          return sdk.transactions
            .transition(
              {
                id: order.id,
                transition: transitions.INSTANT_BOOKING,
                params: orderParams?.params,
              },
              queryParams,
            )
...
// ============ Action ============ //
export const {setInitialValues, clearCheckoutSlice} = checkoutSlice.actions;
