import * as localStore from '@react-native-async-storage/async-storage';
...
const sharetribeTokenStore = ({ clientId }: { clientId: string }) => {
  ...
  const getToken = async () =>
    JSON.parse((await localStore.default.getItem(key)) as string);

  const setToken = async (tokenData: TokenData) =>
    localStore.default.setItem(key, JSON.stringify(tokenData));
  ...
  const removeToken = async () => localStore.default.removeItem(key);
