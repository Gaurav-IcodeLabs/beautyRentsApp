 ...
 */
export const truncateToSubUnitPrecision = (
  inputString,
  subUnitDivisor,
  useComma = false,
) => {
...
export function formatCurrencyAndNumber(amount, currency, minFraction = 2) {
...
export const formatMoney = (value,minFraction) => {
  try {
    if (!(value instanceof Money)) {
      throw new Error('Value must be a Money type')
    }
 ...
 * @return {String} formatted money value
 */
export const formatCurrencyMajorUnit = (
  intl,
  currency,
  valueWithoutSubunits,
) => {
