...
export const supportedCountriesStripe = supportedCountries?.map(c => c.code)
 ...
 */
export const requiredInputs = (countryCode: any) => {
  const bankAccountInputs = stripeCountryConfigs(countryCode)?.accountConfig
  return BANK_ACCOUNT_INPUTS.filter(
    (inputType: any) => bankAccountInputs[inputType],
  )
}
 ...
 */
export const inputTypeToString = (inputType, t) => {
  if (BANK_ACCOUNT_INPUTS.includes(inputType)) {
    return t(`payout.${inputType}.inline`)
  } else {
 ...
 */
export const translateStripeError = (country, stripeError, t) => {
  ...
  if (ibanRequired) {
    return (
      <Text>
        <Trans
          t={t}
          i18nKey="payout.genericStripeErrorIban"
          values={{ country }}
        />
      </Text>
    )
  } else {
    const inputsAsStrings = inputs.map(inputType =>
      inputTypeToString(inputType, t),
    )
    const andTranslated = t('payout.andBeforeLastItemInAList')
    //
    ...
    return (
      <Text>
        <Trans
          t={t}
          i18nKey="payout.genericStripeError"
          values={{ country: country, inputs: inputsInString }}
        />
      </Text>
    )
  }
 ...
 */
export const mapInputsToStripeAccountKeys = (country: any, values: any) => {
 ...
 * @return {Object} key - value in Object literal.
 */
export const formatFieldMessage = (inputType, messageType, t) => {
  if (!BANK_ACCOUNT_INPUTS.includes(inputType)) {
    throw new Error(
      `inputType (${inputType}) must be one of ${BANK_ACCOUNT_INPUTS}`,
    )
  }

  return t(`payout.${inputType}.${messageType}`)
}
 ...
 */
export const cleanedString = (str: any) => {
